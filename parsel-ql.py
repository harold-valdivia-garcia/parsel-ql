...
#
# The references above could not handle some complex arith-expression
# so, I decided to implemented by myself.
...
# Define keywords
(SELECT, FROM, WHERE, AS, NULL, NOT,AND, OR, DISTINCT,
 ALL) = map(CaselessKeyword, """SELECT, FROM, WHERE, AS,
 NULL, NOT, AND, OR, DISTINCT, ALL""".replace(",","").split())

keywords = (SELECT|FROM|WHERE|AS|NULL|NOT|AND|OR|DISTINCT|ALL)
...
bool_term << (Optional(NOT) + bool_factor +
              ZeroOrMore(AND + Optional(NOT) + bool_factor))
